...
class ConvBlock(nn.Module):
    def __init__(self, n_stages, n_filters_in, n_filters_out, normalization='none'):
        ...
        for i in range(n_stages):
            ...
            self.ops.append(ConvBlockOneStage(
                input_channel, n_filters_out, normalization))
...
class TimeStepEmbedding(nn.Module):
    def __init__(self, n_filters_in, n_filters_out, normalization='none',
        ...
        self.emb = ConvBlockTemb(
            1, n_filters_in, n_filters_out, normalization=normalization)

        # 任务嵌入（初始化为零，确保初期不影响原始行为）
        self.task_embedding = nn.Embedding(num_tasks, self.embed_dim_out)
        nn.init.zeros_(self.task_embedding.weight)

        # SATC：SNR 自适应权重（可学习的温度参数）
        self.snr_temperature = nn.Parameter(torch.ones(1))

    def forward(self, x, t=None, image=None, task_id=None, alphas_cumprod=None):
        if t is not None:
        ...
        if task_id is not None:
            h_task = self.task_embedding(task_id)  # [B, embed_dim_out]

            # SATC：只有同时有时间步和 alphas_cumprod 时才做自适应
            if alphas_cumprod is not None and temb is not None:
                snr = alphas_cumprod / (1.0 - alphas_cumprod + 1e-8)  # [B]
                log_snr = torch.log(snr + 1e-8)                           # [B]
                lam = torch.sigmoid(self.snr_temperature * log_snr)    # [B]
                lam = lam.view(-1, 1)                                   # [B,1]
                h_task = lam * h_task                                      # 自适应缩放

            if temb is not None:
                temb = temb + h_task
            else:
                temb = h_task  # 纯分割路径（t=None）
...
class ConvBlockTemb(nn.Module):
    def __init__(self, n_stages, n_filters_in, n_filters_out, normalization='none'):
        ...
        for i in range(n_stages):
            ...
            if i < n_stages-1:
                self.ops.append(ConvBlockTembOneStage(
                    input_channel, n_filters_out, normalization))
            else:
                self.ops.append(ConvBlockTembOneStage(
                    input_channel, n_filters_out, normalization, temb_flag=False))
...
class DownsamplingConvBlockTemb(nn.Module):
    def __init__(self, n_filters_in, n_filters_out, stride=2, normalization='none'):
        ...
        self.ops = nn.ModuleList()
        if normalization != 'none':
            self.ops.append(nn.Conv3d(n_filters_in, n_filters_out,
                            stride, padding=0, stride=stride))
            self.ops.append(nn.ReLU(inplace=True))
            if normalization == 'batchnorm':
                self.ops.append(nn.BatchNorm3d(n_filters_out))
            elif normalization == 'groupnorm':
                self.ops.append(nn.GroupNorm(
                    num_groups=16, num_channels=n_filters_out))
            elif normalization == 'instancenorm':
...
class UpsamplingDeconvBlockTemb(nn.Module):
    def __init__(self, n_filters_in, n_filters_out, stride=2, normalization='none'):
        ...
        self.ops = nn.ModuleList()
        if normalization != 'none':
            self.ops.append(nn.ConvTranspose3d(
                n_filters_in, n_filters_out, stride, padding=0, stride=stride))
            self.ops.append(nn.ReLU(inplace=True))
            if normalization == 'batchnorm':
                self.ops.append(nn.BatchNorm3d(n_filters_out))
...
class Encoder_denoise(nn.Module):
    def __init__(self, n_filters, normalization, has_dropout, dropout_rate):
        ...
        self.has_dropout = has_dropout
        self.block_one_dw = DownsamplingConvBlockTemb(
            n_filters, 2 * n_filters, normalization=normalization)

        self.block_two = ConvBlockTemb(
            2, n_filters * 2, n_filters * 2, normalization=normalization)
        self.block_two_dw = DownsamplingConvBlockTemb(
            n_filters * 2, n_filters * 4, normalization=normalization)

        self.block_three = ConvBlockTemb(
            3, n_filters * 4, n_filters * 4, normalization=normalization)
        self.block_three_dw = DownsamplingConvBlockTemb(
            n_filters * 4, n_filters * 8, normalization=normalization)

        self.block_four = ConvBlockTemb(
            3, n_filters * 8, n_filters * 8, normalization=normalization)
        self.block_four_dw = DownsamplingConvBlockTemb(
            n_filters * 8, n_filters * 16, normalization=normalization)

        self.block_five = ConvBlockTemb(
            3, n_filters * 16, n_filters * 16, normalization=normalization)
        self.dropout = nn.Dropout3d(p=dropout_rate, inplace=True)
...
class Decoder_denoise(nn.Module):
    def __init__(self, n_classes=2, n_filters=16, normalization='none', has_dropout=False, dropout_rate=0.5):
        ...
        self.block_five_up = UpsamplingDeconvBlockTemb(
            n_filters * 16, n_filters * 8, normalization=normalization)

        self.block_six = ConvBlockTemb(
            3, n_filters * 8, n_filters * 8, normalization=normalization)
        self.block_six_up = UpsamplingDeconvBlockTemb(
            n_filters * 8, n_filters * 4, normalization=normalization)

        self.block_seven = ConvBlockTemb(
            3, n_filters * 4, n_filters * 4, normalization=normalization)
        self.block_seven_up = UpsamplingDeconvBlockTemb(
            n_filters * 4, n_filters * 2, normalization=normalization)

        self.block_eight = ConvBlockTemb(
            2, n_filters * 2, n_filters * 2, normalization=normalization)
        self.block_eight_up = UpsamplingDeconvBlockTemb(
            n_filters * 2, n_filters, normalization=normalization)

        self.block_nine = ConvBlockTemb(
            1, n_filters, n_filters, normalization=normalization)
        self.out_conv = nn.Conv3d(n_filters, n_classes, 1, padding=0)
...
class DenoiseModel(nn.Module):
    def __init__(self, n_classes, n_channels, n_filters, normalization, has_dropout, dropout_rate):
        super().__init__()
        self.embedding_diffusion = TimeStepEmbedding(
            n_classes+n_channels, n_filters, normalization=normalization)
        self.encoder = Encoder_denoise(
            n_filters, normalization, has_dropout, dropout_rate)
        self.decoder = Decoder_denoise(
            n_classes, n_filters, normalization, has_dropout, dropout_rate)

    # 🌟 修改3: forward 增加 task_id 参数
    def forward(self, x: torch.Tensor, t, image=None, task_id=None):
        # 传递给底层的 Embedding 层
        x, temb = self.embedding_diffusion(
            x, t=t, image=image, task_id=task_id)
...
class DiffVNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, dropout_rate=0.5):
        ...
        self.embedding = TimeStepEmbedding(
            n_channels, n_filters, normalization=normalization)
        self.decoder_theta = Decoder(
            n_classes, n_filters, normalization, has_dropout, dropout_rate)
        self.decoder_psi = Decoder(
            n_classes, n_filters, normalization, has_dropout, dropout_rate)

        self.denoise_model = DenoiseModel(
            n_classes, n_channels, n_filters, normalization, has_dropout, dropout_rate)

        betas = get_named_beta_schedule(
            "linear", self.time_steps, use_scale=True)
        ...
        # 在 self.sampler = UniformSampler(self.time_steps) 之后加：
        # 注册为 buffer，随模型自动迁移设备（GPU/CPU）
        self.register_buffer(
            'alphas_cumprod',
            torch.from_numpy(self.diffusion.alphas_cumprod).float()
        )
        # shape: [1000]，索引 t ∈ [0,999] 得到对应的 ᾱ_t

        # 暴露噪声调度的 alphas_cumprod，供 SATC 使用
        # 将 numpy array 注册为 buffer（随模型一起保存/移动设备）
        if hasattr(self.diffusion, 'alphas_cumprod'):
            self.register_buffer(
                'alphas_cumprod',
                torch.from_numpy(self.diffusion.alphas_cumprod).float()
            )
        elif hasattr(self.diffusion, 'sqrt_alphas_cumprod'):
            # 有些实现存的是 sqrt 版本
            sqrt_acp = self.diffusion.sqrt_alphas_cumprod
            self.register_buffer(
                'alphas_cumprod',
                torch.from_numpy(sqrt_acp ** 2).float()
            )

    def forward(self, image=None, x=None, pred_type=None,
        ...
        if pred_type == "q_sample":
            ...
            # 取当前 batch 时间步对应的 ᾱ_t，供 SATC 计算 λ(t)
            acp = self.alphas_cumprod[step]   # step: [B]，acp: [B]
            x, temb = self.denoise_model.embedding_diffusion(
                x, t=step, image=image, task_id=task_id, alphas_cumprod=acp
            )
            ...
            x, temb = self.embedding(image, task_id=task_id)
            x1, x2, x3, x4, x5, temb = self.denoise_model.encoder(x, temb)
            ...
            sample_out = self.sample_diffusion.ddim_sample_loop(
                self.denoise_model,
                (image.shape[0], self.n_classes) + image.shape[2:],
                model_kwargs={"image": image, "task_id": task_id}
            )
            return sample_out["pred_xstart"]

    def q_sample(self, x_start, t, noise=None):
        """暴露 q_sample 接口供外部 CVDC 调用"""
        if noise is None:
    ...
    def num_timesteps(self):
        """暴露总时间步数，供 CVDC 计算 t_min"""
        return self.time_steps

    def get_encoder_features(self, x_noisy, t_tensor):
        """
        提取高噪声层次的 Encoder 特征，用于 CVDC 一致性损失。
        
        走 D_xi_l 路径的前半段（embedding_diffusion + encoder），
        不走 decoder，task_id=None 确保域无关性。
        
        关键：embedding_diffusion 需要 image 来拼接，
        传零张量保持形状，不引入真实图像信息。
        """
        B = x_noisy.shape[0]
        # 推断空间尺寸（x_noisy: [B, 11, H, W, D]）
        spatial_shape = x_noisy.shape[2:]   # (H, W, D)

        # dummy image：形状与真实 image 一致，全零不携带信息
        # n_channels=1（灰度），与 __init__ 中的 n_channels 对应
        dummy_image = torch.zeros(
            B, 1, *spatial_shape,
            dtype=x_noisy.dtype, device=x_noisy.device
        )

        # 复用 D_xi_l 路径，但：
        # 1. task_id=None：不注入任务条件，保持域无关
        # 2. dummy_image：只为满足形状要求，不携带语义
        x, temb = self.denoise_model.embedding_diffusion(
            x_noisy,
            t=t_tensor,
            image=dummy_image,
        ...
        # 只跑 encoder，取最深层特征 x5
        x1, x2, x3, x4, x5, temb = self.denoise_model.encoder(x, temb)

        return x5   # [B, C_feat, H//8, W//8, D//8]
